...
function PostItem({ post,updatePostList }) {
        ...
        <div className='p-5 border rounded-lg my-5'>
            ...
            <div className='flex gap-8 mt-4'>
                <div className='flex gap-1 items-center text-gray-500'>
                   {!post.likes?.length?  <svg xmlns="http://www.w3.org/2000/svg"
                   onClick={()=>{onLikeClick(true,post._id)}} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" />
                    </svg> : <svg xmlns="http://www.w3.org/2000/svg" onClick={()=>{onLikeClick(false,post._id)}} viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-red-500">
  ...
  <AlertDialogContent>
    <AlertDialogHeader>
      <AlertDialogTitle className="flex justify-between items-center">Commments <AlertDialogCancel>X</AlertDialogCancel></AlertDialogTitle>
      <AlertDialogDescription>
        <CommentList commentList={post?.comments} updatePostList={()=>updatePostList()}/>
                    ...
                    <Image src={user?.imageUrl}
                        ...
                        className='rounded-full'
                        />
                        <input type='text' value={userInputComment} onChange={(e)=>setUserInputComment(e.target.value)} placeholder='Write a comment' className='w-full p-2 
                         bg-slate-100 border rounded-full px-5 outline-blue-300'/>
                         <Button disabled={!userInputComment} onClick={()=>addComment(post._id)} className="bg-blue-400 text-white rounded-xl p-2 h-8 w-8 hover:bg-blue-600"><Send/></Button>
                    </div>
